# emoji_on_esp32s3_touch_lcd_4.3B

`v1.1.0` 是一个基于 ESP32-S3 4.3 寸触摸屏的机器人表情显示原型，聚焦“表情状态切换 + 触摸交互 + 真实状态接入”的最小可用版本。

## 产品功能

- 基于 LVGL 的机器人表情显示界面
- 支持 `booting`、`idle`、`listening`、`processing`、`speaking`、`happy`、`sleep`、`error` 等基础状态
- 支持 9 宫格触摸交互，不同区域触发不同视觉反馈
- 支持左右看、害羞、说话、睡眠、兴奋等表情切换
- 支持更自然的 gaze 与 blink 动画，左右看与眨眼过渡更加平滑
- 支持通过 USB Serial JTAG 接入真实 `robot_link` 状态源

## 版本详情

### v1.1.0

发布日期：2026-04-06

本版本聚焦显示层卡顿优化与按需刷新调度：

- 将 `display_service_render()` 改成按需进入 LVGL 的调度模式，静止画面下不再每 `16ms` 强制进入整套 UI 更新
- 保留开场 `momo`、眨眼、触摸按压、长按判定和表情过渡时的高频刷新，避免因为节流破坏已有动效
- 基于触摸轮询、临时表情超时、自动 gaze/blink、空闲休眠等时间点预计算下一次刷新时机
- 进一步降低静止待机时的 CPU 占用与无效重绘压力，减少 RGB 屏场景下的卡顿风险
- 将工程版本号更新为 `v1.1.0`

### v1.0.2

发布日期：2026-04-05

本版本聚焦左右看视觉重构与版本可回退性说明：

- 去掉用于左右看表现的白色高光点与叠加遮罩对象
- 不再依赖伪发光叠层来模拟视线，而改为让眼睛、嘴和腮红随表情整体横向偏移
- 将脸部相关对象挂到统一父容器上整体移动，减少多对象分别改坐标带来的抖动与重影
- 将左右看偏移改为固定步进，避免长尾插值导致的拖尾感和卡顿感
- 在后续微调中继续细调 `DISPLAY_FACE_SHIFT_PX` 与 `DISPLAY_FACE_SHIFT_STEP_PX`，兼顾左右看幅度与平滑稳定性
- 保留现有 `look left / look right / blink` 自动行为与 9 宫格触摸映射
- 将工程版本号更新为 `v1.0.2`，用于与 `v1.0.1` 明确区分
- 补充版本说明与回退策略，确保后续发布 `v1.0.2` 时不会覆盖已有 `v1.0.1`

### v1.0.1

发布日期：2026-04-05

本版本为稳定性与观感优化补丁版本：

- 修复 LVGL 高开销阴影绘制导致的 `task_wdt` 卡死问题
- 将动态对象的阴影改为更轻量的伪发光描边，保留层次感同时降低重绘负担
- 减少底部状态文字的重复刷新，避免无意义重绘
- 明确 GT911 使用板级 reset 预选 `0x5D` 地址，消除误导性的 I2C 地址初始化警告

### v1.0.0

发布日期：2026-04-05

本版本完成了项目第一阶段可运行闭环：

- 打通 `robot_link -> emotion_engine -> display_service` 状态链路
- 完成触摸驱动接入与 9 区域交互映射
- 完成基础表情渲染与自动行为调度
- 优化左右看动画，减少瞬时跳变
- 加入眨眼功能，并增强 look/blink 的自然感
- 参考 `emoji/5.html` 持续对齐眼睛比例、视线偏移和整体观感

## 状态输入示例

设备运行后，可通过 USB Serial JTAG 发送一行文本切换状态：

```text
idle
listening
processing
speaking
happy
sleep
error
```

也兼容如下格式：

```text
state=speaking
{"state":"processing"}
```

## 目录说明

- `components/display/`：显示、动画、触摸交互
- `components/emotion/`：情绪状态机
- `components/robot_link/`：外部状态输入接入
- `main/`：主程序入口
- `emoji/`：表情视觉参考稿

## 当前定位

`v1.1.0` 为在 `v1.0.2` 基础上继续打磨显示层流畅度的可演示版本。当前方向是在不砍现有动效的前提下，减少静止画面和稳定状态下的无效刷新，继续巩固 `v1.0.0`、`v1.0.1`、`v1.0.2` 已建立的交互与视觉基础。

## 版本与回退策略

- `v1.1.0` 不应覆盖 `v1.0.2`：正确做法是新增一次提交，并单独创建 `v1.1.0` 标签
- 只要保留 `v1.0.2` 的提交或标签，后续即使 `v1.1.0` 效果不满意，也可以随时回到 `v1.0.2`
- 推荐发布顺序：先提交 `v1.1.0` 代码，再打 `v1.1.0` 标签，再推送到远端
- 如果验证失败，可直接检出 `v1.0.2` 标签，或基于 `v1.0.2` 新开修复分支继续调整
